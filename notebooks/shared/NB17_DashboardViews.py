...
WITH latest AS (
  SELECT *, ROW_NUMBER() OVER (
      PARTITION BY source_table_id ORDER BY {LATEST_RUN_ORDER}) AS rn
...
SELECT c.source_table_id, c.connection_id,
  concat_ws('.', c.target_catalog, c.target_schema, c.target_table) AS bronze_target,
...
latest_etl_run AS (
  SELECT source_table_id, run_id, ROW_NUMBER() OVER (
      PARTITION BY source_table_id ORDER BY {LATEST_RUN_ORDER}) AS rn
  FROM {ctrl('table_run_log')}
  WHERE operation IN ('ETL_FULL','ETL_INCREMENTAL')
),
...
SELECT c.source_table_id, c.connection_id, c.source_schema, c.source_table,
...
LEFT JOIN latest_etl_run ler ON ler.source_table_id = c.source_table_id AND ler.rn = 1
LEFT JOIN q ON q.run_id = ler.run_id AND q.source_table_id = c.source_table_id
""")
